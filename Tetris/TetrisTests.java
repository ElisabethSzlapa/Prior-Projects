...
public class TetrisTests {
    ...
    @Test
    void testPlacePiecesMessy() {

        TetrisBoard board = new TetrisBoard(10,24);
        TetrisPiece pieceA = new TetrisPiece(TetrisPiece.SQUARE_STR);
        TetrisPiece pieceB = new TetrisPiece(TetrisPiece.L2_STR);
        TetrisPiece pieceC = new TetrisPiece(TetrisPiece.L1_STR);
        TetrisPiece pieceD = new TetrisPiece(TetrisPiece.STICK_STR);

        board.commit();
        int retval = board.placePiece(pieceA, 0,0);
        //fill the entire row
        retval = board.placePiece(pieceA, 2,0); board.commit();
        retval = board.placePiece(pieceC.computeNextRotation().computeNextRotation().computeNextRotation(), 4,0); board.commit();
        retval = board.placePiece(pieceB, 7,0); board.commit();
        retval = board.placePiece(pieceD, 9,0); board.commit();
        assertEquals(TetrisBoard.ADD_ROW_FILLED, retval);
