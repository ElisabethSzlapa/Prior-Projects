...
class Monitor:
    ...
    def _average_total_distance(self) -> float:
        ...
        tot_dist = 0
        driver_count = 0
        for driver_activities in self._activities[DRIVER].values():
            prev_loc = None
            driver_count += 1
            for activity in driver_activities:
                curr_loc = activity.location
                if prev_loc is not None:
                    tot_dist += monitor_manhattan(curr_loc, prev_loc)
                prev_loc = curr_loc

        if driver_count != 0:
            return tot_dist / driver_count
        return 0
    ...
    def _average_ride_distance(self) -> float:
        ...
        for driver_activities in self._activities[DRIVER].values():
            ...
            for activity in driver_activities:
                ...
                if activity.description is PICKUP:
                    start_loc = activity.location
                    ongoing_request = True

                if (activity.description is DROPOFF and ongoing_request
                        and start_loc is not None):
                    end_loc = activity.location
                    tot_dist += monitor_manhattan(start_loc, end_loc)
                    ongoing_request = False
...
def monitor_manhattan(origin: Location, destination: Location) -> int:
    """
