    ...
    def run(self, initial_events: List[Event]) -> Dict[str, float]:
        ...
        while not self._events.is_empty():
            ...
            if new_events is not None:
                if len(new_events) == 2:  # pickup and cancellation
                    self._monitor.notify(new_events[0].timestamp, "driver",
                                         "pickup", new_events[0].driver.id,
                                         new_events[0].rider.origin)

                    new_events = [comp_pickup_cancel(new_events[0],
                                                     new_events[1])]
...
def comp_pickup_cancel(pickup: Event, cancellation: Event) -> Event:
    """
    Takes a pickup event and a cancellation event, and returns the event with
    the earlier timestamp. If pickup and cancellation have the same timestamp,
    the pickup event is returned,
    """

    if pickup.timestamp <= cancellation.timestamp:
        return pickup
    return cancellation
