...
def _load_data() -> List[PopulationTree]:
    ...
    rec_countries = recorded_countries(country_populations)
    region_list = []
    for region in regions:
        region_population = 0
        countries = []
        for country in regions[region]:
            if country in rec_countries and country_populations[country] is not\
                    None:
                region_population += country_populations[country]
                countries.append(PopulationTree(False, country, [],
                                                country_populations[country]))
        region_list.append(PopulationTree(False, region, countries,
                                          region_population))
    return region_list


def recorded_countries(country_population: Dict[str, int]) -> List[str]:
    """
    Returns a list of coutries with recorded populations in country_population
    """
    rec_countries = []
...
def _get_population_data() -> Dict[str, int]:
    ...
    for item in population_data:
        countries[item['country']['value']] = item['value']
...
def _get_region_data() -> Dict[str, List[str]]:
    ...
    for item in country_data:
        if item['region']['id'] != 'NA':
            if item['region']['value'] not in established_regions:
                regions[item['region']['value']] = [item['name']]
                established_regions.append(item['region']['value'])
            else:
                regions[item['region']['value']].append(item['name'])
